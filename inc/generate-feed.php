...
//        if($prodID==804) $this->_storedebug( $this->_metaData);
    ...
    private function _product_get_description($product){
        $desc = apply_filters('the_content', version_compare( WC_VERSION, '3.0', '>=' ) ? $product->get_description() : $product->post->post_content );
        if (empty($desc)) {
            return apply_filters('feed_the_content', $desc, $product->post );
        }
        return $desc;
    }
